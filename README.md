...
The gate runs `curl /health`, fast pytest (`test_sync_quality_unit` + `test_api_smoke` + `test_cleanup` + `test_async_jobs`), then the live MP3 check. Set `SKIP_LIVE_SYNC=1` to skip Whisper. Do **not** pipe `live_sync_quality_check.py` through `tee` — that breaks exit-code checks.

Same pytest bundle without live sync:
